...
class Lime_Explanation:
    ...
    def __init__(self, model, class_names, random_state = None, num_samples=500, num_features=10, bow = True):
        self.model = model
    ...
    def explain(self, lime_input):

        # TODO. parameter richtig setzen 
        explainer = lime_text.LimeTextExplainer(class_names=self.class_names, random_state=self.random_state, bow = self.bow)
    ...
    def prediction_function_qa(self, question_and_context_list):
        ...
        for element in question_and_context_list:
            ...
            all_tokens = []
            model_output, all_tokens, _ = self.model.predict(question,context)
